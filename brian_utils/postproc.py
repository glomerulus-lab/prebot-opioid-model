...
#     #quiescent.astype(int)
#     burster = np.where(cov > 1.)[0].astype(int)
#     tonic = np.where(cov < 1).[0].astype(int)
    
#     cell_type[0] = np.where(cov > 1.)[0] # Bursters
#     cell_type[1] = np.where(cov < 1.)[0]  # Tonic
#     cell_type[2] = quiescent  # Quiescent
#     return (cell_type)
...
def pop_burst_stats(t,rate,scale=False,height=4,prominence=10,width=0.1,distance=0.5,rel_height=0.9):
    '''
    ...
    df['PBI'] = PBI
    df['IBI'] = IBI
    df['Frequency'] = np.append(1/np.diff(pk_t), np.nan)
    df['Period'] = 1/np.append(1/np.diff(pk_t), np.nan)
    df['Peaks'] = rate[pks] / Hz

    return(df)

def irregularity(a):
    irs = (np.sum(np.abs(np.diff(a)) / np.abs(a)[:-1])) / a.size
    return irs
