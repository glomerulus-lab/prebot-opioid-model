...
def one_network_df(run_seed, exp_type):
    ...
    files = glob.glob(f'{exp_type}_pkls/seed{run_seed}-gnap*.pkl')
...
def concat_networks(seed_list, exp_type):
    ...
    df.reset_index()
    df.to_csv(f'seed{seed_list[0]}to{seed_list[len(seed_list)-1]}_con20_burst_stats.csv')
    
@click.command()
@click.argument('first_seed', type = int)
@click.argument('last_seed', type = int)
@click.option('--exp_type', type = str, default = 'gnap_mod') # gnap_mod, high_con, or targeted_damgo
...
# need seed26-gnap03, seed29-gnap05, seed39-gnap01, seed8 all but gnap-05, seed9 all