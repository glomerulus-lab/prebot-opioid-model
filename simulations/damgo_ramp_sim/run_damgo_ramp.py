...
@click.option('--pct_connect', type = float, default = 0.01, show_default = True)
def main(prefix, run_seed, g_nap_str, idx, hyp_opioid, syn_shut, pct_connect):
    ...
    os.mkdir(f'damgo_ramp_output_plots/pcon_{pct_connect}/{prefix}')
    basename='test'
    path = '/research/harris/prebot_opioid_model/data'
    savename= f'{path}/damgo_ramp_pkls/pcon_{pct_connect}/{prefix}_vars.pkl'
    ...
    we_max = 3.5 / (pct_connect * 100) # Excitatory synaptic strength (nS)
    wi_max = 3.5 / (pct_connect * 100) # Inhibitory synaptic strength (nS)
    ...
    plt.axis('equal')
    plt.tight_layout()
    plt.savefig(f'damgo_ramp_output_plots/pcon_{pct_connect}/{prefix}/{prefix}_clouds.png')
    ...
    sns.despine()
    plt.savefig(f'damgo_ramp_output_plots/pcon_{pct_connect}/{prefix}/{prefix}_perts.png')
...
#     net.run(20*second,report='text')
    ...
    plt.savefig(f'damgo_ramp_output_plots/pcon_{pct_connect}/{prefix}/{prefix}_trace.png',dpi=300)
...
#     plt.savefig(f''damgo_ramp_output_plots/pcon_{pct_connect}/{prefix}/{prefix}_synblock.png')
    ...
    plt.savefig(f'damgo_ramp_output_plots/pcon_{pct_connect}/{prefix}/{prefix}_high_res_trace.png', dpi=300)
