...
def main(start_seed, end_seed, pct_connect):
    df = pd.read_csv(f'seed{start_seed}to{end_seed}_damgo_ramp_{pct_connect}.csv')
    ld50s = []
    for seed in range(1,41):
        # change filepath and/or naming accordingly (this can be a common cause of bugs)
        with open(f'../../../../prebot_opioid_data/damgo_ramp_pkls/pcon_{pct_connect}/seed{seed}-damgo_ramp_pcon_{pct_connect}_vars.pkl','rb') as fid:
        ...
        plt.vlines(popt[1], 0, 40, linestyles = 'dashed', colors = 'r')
        plt.xlabel(r'$I_{opioid}$ (pA)')
        plt.ylabel('FR \n(Hz/cell)')
        plt.title('Max firing rates of each dosage level')
        plt.savefig(f'{pct_connect}_seed{seed}_max_fr.png')
    
    # add LD50s to dataset
    df['LD50'] = ld50s
    df.to_csv(f'seed{start_seed}to{end_seed}_damgo_ramp_{pct_connect}.csv')
