...
@click.option('--run_seed', type = int, default = 1, show_default = True) # 2nd command line argument: setting the seed
@click.option('--g_leak_str', type = float, default = 0, show_default = True) # 3rd command line argument: setting g_leak perturbation strength
...
@click.option('--target', type = str, default = 'high', show_default = True)
@click.option('--synblock', type = int, default = 1, show_default = True)
@click.option('--condition', type = str, default = 'control', show_default=True)
def main(prefix, run_seed, g_leak_str, idx, hyp_opioid, syn_shut, target, synblock, condition):
    ...
    os.mkdir(f'targeted_damgo_output_plots/{target}_gleak_figs/{prefix}')
    basename='test'
    savename= f'{path}/data/targeted_damgo_pkls/{target}_gleak_pkls/{prefix}_vars.pkl'
    ...
    eqs_yaml = '../../harris_eqs_oprmv1_gleak.yaml'
    ...
    if target == 'low':
        ...
        Pe = neurons[n_inh:n_oprm1+n_inh] # Excitatory oprm1- neurons
        Pe_oprm1 = neurons[n_oprm1+n_inh:] # Excitatory oprm1+ neurons
    ...
    if synblock == 1:
        op_vals  =                 np.array([0,0,1,1]) # When are opioids applied?
        g_leak_mult = g_leak_str * np.array([0,0,0,0]) + 1 # multiply by g_leak_str to change strength of effect

        we =                     np.array([1,1,1,1]) # When are excitatory synapses active? (always)
        wi =                     np.array([1,1,1,1]) # When are inhibitory synapses active
    elif synblock == 0 and condition == 'control':
        op_vals  =                 np.array([0,0]) # When are opioids applied?
        g_leak_mult = g_leak_str * np.array([0,0]) + 1 # multiply by g_leak_str to change strength of effect

        we =                     np.array([0,0]) # When are excitatory synapses active? (always)
        wi =                     np.array([0,0]) # When are inhibitory synapses active
    
    elif synblock == 0 and condition == 'DAMGO':
        op_vals  =                 np.array([1,1]) # When are opioids applied?
        g_leak_mult = g_leak_str * np.array([0,0]) + 1 # multiply by g_leak_str to change strength of effect
    ...
    g_l_drug = TimedArray(g_leak_mult, dt = perturbation_dt)
    ...
    modified_gl_vals = (g_leak_str + 1) * gl_vals
    plt.figure(figsize=(4,4))
    plt.plot(gl_vals/nS, gnap_vals/nS, 'k.', alpha = 0.4)
    plt.plot(modified_gl_vals/nS, gnap_vals/nS, 'r.', alpha=0.4)
    plt.tight_layout()
    plt.xlabel(r'$G_{Leak} (nS)$')
    plt.ylabel(r'$G_{NaP} (nS)$')
    plt.axis('equal')
    plt.tight_layout()
    plt.savefig(f'targeted_damgo_output_plots/{target}_gleak_figs/{prefix}/{prefix}_mod_gnap_gleak.pdf')
    ...
    sns.despine()
    plt.savefig(f'targeted_damgo_output_plots/{target}_gleak_figs/{prefix}/{prefix}_gnap_pert.pdf')
    ...
    print(neurons.g_l[0:60]/nS)
    print(neurons.g_l[60:180]/nS)
    print(neurons.g_l[180:300]/nS)
    ...
    print(neurons.damgo_sensitivity[0:60])
    print(neurons.damgo_sensitivity[60:180])
    print(neurons.damgo_sensitivity[180:300])
    ...
    ax4.plot(tt,g_l_drug(tt)/nS)
    tvec = np.arange(0,runtime/second,.1)*second
    ax4.set_ylabel(r'G_{Leak}',fontsize=8)
    ...
    plt.savefig(f'targeted_damgo_output_plots/{target}_gleak_figs/{prefix}/{prefix}_trace.pdf')
...
#     plt.savefig(f'targeted_damgo_output_plots/{target}_gleak_figs/{prefix}/{prefix}_sanity_check.pdf')

    # visualize synaptic block
    #plt.figure(figsize=(4,4))
    #for ii in range(15):
    ...
    #plt.savefig(f'gleak_mod_figs/{prefix}/{prefix}_synblock.pdf')
