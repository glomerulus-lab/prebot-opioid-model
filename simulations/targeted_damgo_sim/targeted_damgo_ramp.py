...
@click.option('--g_leak_str', type = float, default = 1, show_default = True) # 3rd command line argument: setting g_leak perturbation strength
@click.option('--idx', type = int, default = 1, show_default = True)
...
def main(prefix, run_seed, g_leak_str, idx, hyp_opioid, syn_shut, target):
    ...
    os.mkdir(f'targeted_damgo_output_plots/damgo_to_{target}_gl_output_plots/{prefix}')
    basename='test'
    savename= f'{path}/data/targeted_damgo_ramp_pkls/damgo_to_{target}_gl_ramp_pkls/{prefix}_vars.pkl'
    ...
    g_leak_mult = np.zeros(len(op_vals)) + 1 # multiply by g_leak_str to change strength of effect
    ...
    plt.plot(modified_gl_vals/nS, gnap_vals/nS, 'r.', alpha=0)
    plt.tight_layout()
    plt.xlabel(r'$G_{Leak} (nS)$')
    plt.ylabel(r'$G_{NaP} (nS)$')
    plt.axis('equal')
    plt.tight_layout()
    plt.savefig(f'targeted_damgo_output_plots/damgo_to_{target}_gl_output_plots/{prefix}/{prefix}_mod_gnap_gleak.pdf')
    ...
    sns.despine()
    plt.savefig(f'targeted_damgo_output_plots/damgo_to_{target}_gl_output_plots/{prefix}/{prefix}_gnap_pert.pdf')
...
#     print(neurons.damgo_sensitivity[180:300])
    ...
    plt.savefig(f'targeted_damgo_output_plots/damgo_to_{target}_gl_output_plots/{prefix}/{prefix}_trace.pdf')
    ...
    plt.savefig(f'targeted_damgo_output_plots/damgo_to_{target}_gl_output_plots/{prefix}/{prefix}_sanity_check.pdf')
