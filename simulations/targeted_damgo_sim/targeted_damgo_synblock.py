...
@click.option('--high_or_low', type = str, default = 'high', show_default = True)
@click.option('--run_seed', type = int, default = 0, show_default = True) # 2nd command line argument: setting the seed
@click.option('--g_leak_str', type = float, default = 1, show_default = True) # 3rd command line argument: setting g_nap strength
...
def main(prefix, high_or_low, run_seed, g_leak_str, idx, hyp_opioid, syn_shut, struct, condition):
    ...
    os.mkdir(f'targeted_damgo_synblock_output_plots/{struct}_figs/{prefix}')
    basename='test'
    savename= f'{path}/data/targeted_damgo_synblock_pkls/{struct}_pkls/{prefix}_vars.pkl'
    ...
    if high_or_low == 'low':
        ...
        Pe = neurons[n_inh:N] # Excitatory neurons
        order = np.argsort(Pe.g_l/nS) # Order excitatory neurons by g_leak (ascending) 
        Pe.g_l = Pe.g_l[order]
        Pe.g_nap = Pe.g_nap[order]
        Pe_oprm1 = Pe[:int(n_excit/2)] # Excitatory oprm1+ neurons
        Pe = Pe[int(n_excit/2):n_excit] # Excitatory oprm1- neurons
    else:
        Pi = neurons[:n_inh] # Inhibitory neurons
        Pe = neurons[n_inh:N] # Excitatory neurons
        order = np.argsort(Pe.g_l/nS) # Order excitatory neurons by g_leak (ascending) 
        Pe.g_l = Pe.g_l[order]
        Pe.g_nap = Pe.g_nap[order]
        Pe_oprm1 = Pe[int(n_excit/2):n_excit] # Excitatory oprm1+ neurons
        Pe = Pe[:int(n_excit/2)] # Excitatory oprm1- neurons
    ...
    if condition == 'Control':
        ...
        g_leak_mult = g_leak_str * np.array([0,0]) + 1
    
    elif condition == 'DAMGO':
        op_vals = np.array([1,1])
        g_leak_mult = g_leak_str * np.array([0,0]) + 1
    
    elif condition == 'G_Leak':
        op_vals = np.array([0,0])
        g_leak_mult = g_leak_str * np.array([1,1]) + 1
        
    elif condition == 'DAMGO+G_Leak':
        op_vals = np.array([1,1])
        g_leak_mult = g_leak_str * np.array([1,1]) + 1
    ...
    plt.axis('equal')
    plt.tight_layout()
    plt.savefig(f'targeted_damgo_synblock_output_plots/{struct}_figs/{prefix}/{prefix}_gnap_gleak.png')
    ...
    sns.despine()
    plt.savefig(f'targeted_damgo_synblock_output_plots/{struct}_figs/{prefix}/{prefix}_gnap_pert.png')
...
#     net.run(20*second,report='text')
    ...
    ax1.set_ylabel(r'$g_{syn}^{opioid}$',fontsize=8)
    ...
    # plt.ylabel('DAMGO (percent max block)')
    ax2.set_ylabel(r'$I_{opioid}$ (pA)',fontsize=8)
    ...
    plt.savefig(f'targeted_damgo_synblock_output_plots/{struct}_figs/{prefix}/{prefix}_gnap_trace.png')
